...
public class lb_Bird : MonoBehaviour {
	...
	IEnumerator FlyToTarget(Vector3 target){
		...
		var vectorDirectionToTarget = (target-transform.position).normalized;
		...
		var flyingForce = 50.0f*controller.birdScale;
		while(true){
			if(!paused){
		...
		var hitColliders = Physics.OverlapSphere(target,0.05f*controller.birdScale);
	...
	float FindBankingAngle(Vector3 birdForward, Vector3 dirToTarget){
		var cr = Vector3.Cross (birdForward,dirToTarget);
		var ang = Vector3.Dot (cr,Vector3.up);
		return ang;
	}
	...
	void Flee(){
		if(!dead){
			...
			var farAwayTarget = transform.position;
