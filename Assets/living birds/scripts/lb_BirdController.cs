...
public class lb_BirdController : MonoBehaviour {
	...
	void Start () {
		...
		var groundTargets = GameObject.FindGameObjectsWithTag("lb_groundTarget");
		var perchTargets = GameObject.FindGameObjectsWithTag("lb_perchTarget");
		...
		for (var i=0;i<perchTargets.Length;i++){
		...
		var fEmitter = Resources.Load ("featherEmitter",typeof(GameObject)) as GameObject;
		for(var i=0;i<3;i++){
			featherEmitters[i] = Instantiate (fEmitter,Vector3.zero,Quaternion.identity);
	...
	IEnumerator UpdateTargets(){
		var gtRemove = new List<GameObject>();
		var ptRemove = new List<GameObject>();

		while(true){
			gtRemove.Clear();
			ptRemove.Clear();
			//check targets to see if they are out of range
			for(var i=0;i<birdGroundTargets.Count;i++){
			...
			for (var i=0;i<birdPerchTargets.Count;i++){
			...
			foreach (var entry in gtRemove){
				birdGroundTargets.Remove(entry);
			}
			foreach (var entry in ptRemove){
				birdPerchTargets.Remove(entry);
			}
			yield return 0;
			//now check for any new Targets
			var hits = Physics.OverlapSphere(currentCamera.transform.position,unspawnDistance);
	...
	bool BirdOffCamera(Vector3 birdPos){
		var screenPos = currentCamera.WorldToViewportPoint(birdPos);
	...
	void SpawnBird(){
		if (!pause){
			...
			var randomBirdIndex = Mathf.FloorToInt (Random.Range (0,myBirds.Length));
			var loopCheck = 0;
	...
	Vector3 FindPositionOffCamera(){
		...
		var ray = -currentCamera.transform.forward;
		var loopCheck = 0;
	...
	void BirdFindTarget(GameObject bird){
		...
		if (birdGroundTargets.Count > 0 || birdPerchTargets.Count > 0){
			...
			var gtArea=0.0f;
			var ptArea=birdPerchTargets.Count*0.3f;
	...
	void FeatherEmit(Vector3 pos){
		foreach (var fEmit in featherEmitters){
			if(!fEmit.activeSelf){
