...
namespace AdvancedTerrainGrass {
	...
	public class GrassManagerEditor : Editor {
 	...
 	//	GUI
		...
		private GUIStyle myMiniBtn;
		private GUIStyle mainFoldoutStyle;
		private GUIStyle myMiniHelpBtn;
...
//	Styles -------------------
			...
			var myCol = new Color (1.0f,0.8f,0.0f,1.0f);
			var myCol01 = new Color(0.30f,0.47f,1.0f,1.0f); // matches highlight blue //new Color(1.0f,0.3f,0.0f,1.0f); // Orange
			...
			var mainLabelStyle = new GUIStyle(EditorStyles.label);
			mainLabelStyle.normal.textColor = primaryCol;

			var primaryLabelStyle = new GUIStyle(EditorStyles.label);
		...
		//	Render Settings
			...
			EditorGUILayout.BeginVertical(aBox);
	            ...
	            if (Foldout_Rendersettings.boolValue) {
	                ...
	                var selectedLayer = CameraLayer.intValue;
		...
		//	Prototypes
			...
			EditorGUILayout.BeginVertical(aBox);
				...
				//	Second check if all meshes are actually assigned
					...
					if (items > 0) {
						var toolbarStrings = new[] {"Single Prototype", "All Prototypes"};
						LayerEditMode.intValue = GUILayout.Toolbar(LayerEditMode.intValue, toolbarStrings);
						GUILayout.Space(4);

						var previews = new Texture2D[items];
						for(var i = 0; i < items; i++) {
							if (terData.detailPrototypes[i].prototype != null) {
			     				previews[i] = AssetPreview.GetAssetPreview(terData.detailPrototypes[i].prototype);
			     			}
							else {
								previews[i] = AssetPreview.GetMiniThumbnail(terData.detailPrototypes[i].prototypeTexture);
							}
						}
...
//	Show single Layer
							...
							var cols = Mathf.FloorToInt( (EditorGUIUtility.currentViewWidth - thumbSize) / thumbSize );
							var rows = Mathf.CeilToInt ( (float)items / cols);
...
//	Show all Layers
            ...
            if (oldDrawInEditMode == false && DrawInEditMode.boolValue)
            {
            ...
            else if (oldDrawInEditMode && DrawInEditMode.boolValue == false)
            {
...
// --------------------------------------------
		...
		private void ToggleGrid() {
			...
			var script = (GrassManager)target;
			var ter = script.GetComponent<Terrain>();
	        var terData = ter.terrainData;

	        var id = ter.transform.gameObject.GetInstanceID();
...
// --------------------------------------------
		...
		private void GetPrototypes() {
	        ...
	        var oldNumberOfLayers = v_mesh.arraySize;
...
//	Here we deal with detail meshes
		    ...
		    //	Add default Quad for simple Grass Textures
        		...
        		//	Most common is width * height
        			...
        			var guIDS = AssetDatabase.FindAssets (MeshToAssign);
	        		if (guIDS.Length > 0) {
		    ...
		    //	Update Colors
		    	if(!FreezeSizeAndColor.boolValue) {
			    	if (v_mat.GetArrayElementAtIndex(i).objectReferenceValue != null) {
			    		var Mat = (Material) v_mat.GetArrayElementAtIndex(i).objectReferenceValue as Material;
...
//	It merges density maps and configures Cells and CellContents and writes all out to disc.
	...
	public void SaveTerrainData() {
	        ...
	        var TerrainDetailSize = Vector2.zero;
	    ...
	    //  We assume squared Terrains here...
            ...
            var NumberOfBucketsPerCell = NumberOfBucketsPerCellEnum.intValue;
        ...
        //  Check if we have to merge detail layers
            ...
            var tCells = new GrassCell[NumberOfCells * NumberOfCells];
        //  As we do not know the final cout we create cellcontens in a temporary list
            ...
            var tmapByte = new byte[NumberOfLayers][];
                ...
                //  Check merge
                    ...
                    for (var x = 0; x < (int)TerrainDetailSize.x; x++ ){
                        for (var y = 0; y < (int)TerrainDetailSize.y; y ++) {
							...
							tmapByte[layer][ x * (int)TerrainDetailSize.y + y ] = Convert.ToByte( temp[0,0] );                            
                        //  Merge
                            if(doMerge) {
                                ...
                                //  Check DoSoftMerge as otherwise even soft merged layers would be added
                                    if (MergeArray[layer][m] != 0 && !DoSoftMerge.GetArrayElementAtIndex(MergeArray[layer][m] - 1).boolValue ) {
										...
										tmapByte[layer][ x * (int)TerrainDetailSize.y + y ] = Convert.ToByte( tmapByte[layer][ x * (int)TerrainDetailSize.y + y ] + temp[0,0] );                                      
                                    }
                                }
                            }  //  End of merge
                        }
        ...
        //	Copy data to Scriptable Object
			...
			var terPath = Path.GetDirectoryName( AssetDatabase.GetAssetPath(terData) );
			var terName = Path.GetFileNameWithoutExtension( AssetDatabase.GetAssetPath(terData) );
			if(rtUseCompute.boolValue) {
