...
namespace AdvancedTerrainGrass {
    ...
    public class AtgHiZBuffer : MonoBehaviour
    {
        ...
        private void OnPreRender() {
            ...
            var sizeX = camera.pixelWidth;
            var sizeY = camera.pixelHeight;
            m_LODCount = (int)Mathf.Floor(Mathf.Log(sizeX, 2f));
            var isCommandBufferInvalid = false;
...
// was Float
            ...
            if (m_CommandBuffer == null || isCommandBufferInvalid) {
                ...
                var id = new RenderTargetIdentifier(m_HiZ);
                m_CommandBuffer.Blit(null, id, material, (int)Pass.Blit);

                for (var i = 0; i < m_LODCount; ++i) {
