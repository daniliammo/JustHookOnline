...
//  ////////////////////////////////////////////////////
        ...
        public static float OneOverInt32MaxVal = 1.0f / uint.MaxValue;
...
//  VR
        ...
        private uint[] t_args = new uint[5] { 0, 0, 0, 0, 0 };
...
private void Cleanup() {
        ...
        //  Release ComputeBuffers
            ...
            if (MergedCellcontentsBuffer != null) {
                for (var i = 0; i < MergedCellcontentsBuffer.Length; i++) {
            ...
            if (argsBuffer != null) {
                for (var i = 0; i < argsBuffer.Length; i++) {
...
//  Function to change rendering settings at run time
        ...
        //  Update Culling Group
            var distances = new[] {CurrentCullDistance * CullingGroupDistanceFactor, CurrentCacheDistance * CullingGroupDistanceFactor };
        ...
        //  Update fade distances in shaders
            ...
            for (var i = 0; i < Cells.Length; i ++) {
                ...
                if (CurrentCell.state == 3) {
                    ...
                    for (var j = 0; j < NumberOfLayersInCell; j++) {
        ...
        //  Update Compute Buffer
            if (UseCompute) {
                ...
                for (var i = 0; i < OrigNumberOfLayers; i++) {
...
//  Init Cells, CellContens and Density Maps from Scriptable Object
        ...
        public void InitCellsFast() {
            ...
            for (var i = 0; i < SavedTerrainData.DensityMaps.Count; i ++) {
            ...
            var CellContentIndex = 0;

        //  Get TerrainShift as the terrain might not be positioned at the same spot were the data was baked.
            var TerrainShift = SavedTerrainData.TerrainPosition - TerrainPosition;

        //  We really have to copy the arrays as otherwise material, mesh or argsbuffer error?!
            var cLength = SavedTerrainData.Cells.Length;
            for(var CurrrentCellIndex = 0; CurrrentCellIndex < cLength; CurrrentCellIndex++) {
            ...
            //  Handle shifted terrains
                ...
                var CellContentsinCell = Cells[CurrrentCellIndex].CellContentCount;
                for (var layer = 0; layer < CellContentsinCell; layer++) {
...
//  Init Cells, CellContens and Density Maps directly at start
        ...
        //  Merge Layers
            ...
            var MergeArray = new int[OrigNumberOfLayers][]; // Actually too big...
            var SoftMergeArray = new int[OrigNumberOfLayers][]; // Actually too big...
            ...
            for (var i = 0; i < OrigNumberOfLayers; i ++) {
                ...
                //  Check if the Layer we want to merge with does not get merged itself..
                    ...
                    //  Find a the first free entry Merge
                        for (var j = 0; j < OrigNumberOfLayers - 1; j++) {
        ...
        //  As we do not know the final count we create cellcontents in a temporary list
            var tempCellContent = new List<GrassCellContent>();
        ...
        //  TODO: remapping of layers did not work out. So we do it brute force...
            ...
            //  Skip mapByte if layer uses hard merge
                ...
                //  Check merge. Only merge densities if SoftMerge is disabled.
                    ...
                    for (var x = 0; x < (int)TerrainDetailSize.x; x++ ){
                        for (var y = 0; y < (int)TerrainDetailSize.y; y ++) {
                            // flipped!!!!!????????? no,not in this case.
                            var temp = terData.GetDetailLayer(x, y, 1, 1, layer );
...
*/
                            mapByte[layer][ x * (int)TerrainDetailSize.y + y ] = Convert.ToByte( temp[0,0] );
                        //  Merge
                            if(doMerge) {
                                for (var m = 0; m < OrigNumberOfLayers - 1; m++ ) {
                                //  Check DoSoftMerge as otherwise even soft merged layers would be added
                                    if (MergeArray[layer][m] != 0 && !DoSoftMerge[MergeArray[layer][m] - 1]) {
                                        ...
                                        mapByte[layer][ x * (int)TerrainDetailSize.y + y ] = Convert.ToByte( mapByte[layer][ x * (int)TerrainDetailSize.y + y ] + temp[0,0] );  
                                    }
                                }
                            }
                        }
            ...
            var density = 0;
            var CellContentOffset = 0;
            Vector3 Center;
            var ArrayOffset = 0;

            for(var x = 0; x < NumberOfCells; x++) {
                for(var z = 0; z < NumberOfCells; z++) {
                ...
                //  Create and setup all CellContens (layers) for the given Cell
                    ...
                    //  Only create cellcontent entry if the layer does not get merged.
                        ...
                        //  Sum up density for all buckets 
                            for(var xp = 0; xp < NumberOfBucketsPerCell; xp++) {
                        ...
                        //  Check for softly merged layers
                            var NumberOfSoftlyMergedLayers = 0;

                            if(SoftMergeArray[layer] != null) {
                                for (var l = 0; l < OrigNumberOfLayers - 1; l++) {
                                    if ( SoftMergeArray[layer][l] == 0) {
                                        break;
                                    }
                                    var softMergedLayer = SoftMergeArray[layer][l] - 1; // as index starts at 1
                                    var softDensity = 0;
                        ...
                        //  Skip Content if density = 0
                            ...
                            //  Softly merged Layers
                                if( NumberOfSoftlyMergedLayers > 0 ) {
                                    var tempSoffMergedLayers = new List<int>();
                                    for (var l = 0; l < OrigNumberOfLayers - 1; l++) {
                                        if(SoftMergeArray[layer][l] != 0) {
...
//  Init Grid, Culling groups and all the rest
        ...
        //  Due to ExecuteInEditMode LayerToMergeWith might not be setup by the editor. So we will simply create and fill it
            var updateLayerToMergeWith = false;
            ...
            for (var i = 0; i < NumberOfFinalLayers; i++) {
        ...
        //  Get Heights
            ...
            for(var x = 0; x < TerrainHeightmapWidth; x++) {
                for(var z = 0; z < TerrainHeightmapHeight; z++) {
...
//  Function to immediately init all cells within the user defined BurstRadius
        ...
        //  Loop over all Cells
            var t_NumberOfCells = Cells.Length;
            for (var i = 0; i < t_NumberOfCells; i ++) {
                var CurrentCell = Cells[i];
                var distance = Vector3.Distance(camPosition, CurrentCell.Center);
                int Layer;
...
private void DrawGrass() {
                        ...
                        for (var j = 0; j < NumberOfLayersInCell; j++) {
                            ...
                            if (!UseCompute) {
                                ...
                                //  Set target Buffer
                                    ...
                                    var threadGroups = Mathf.CeilToInt(CurrentCellContent.Instances / 64.0f); // 1024.0f);
        ...
        //  Compute: Draw grass
            if (UseCompute) {
                ...
                for (var i = 0; i < OrigNumberOfLayers; i ++) {
                    var layer = FinalLayersIndices[i];
                //  Skip merged layers
                    if (layer == -1) {
                        continue;
                    }
...
//  Bilinear sampling from the height array
        ...
        //  Adjust x positions to match our array
            ...
            var HeightSampleLowerRow =  TerrainHeights [ normalizedHeightPosLower_x + normalizedHeightPosLower_y ] * Upperx;
                  HeightSampleLowerRow += TerrainHeights [ normalizedHeightPosUpper_x + normalizedHeightPosLower_y ] * Lowerx;
            var HeightSampleUpperRow =  TerrainHeights [ normalizedHeightPosLower_x + normalizedHeightPosUpper_y ] * Upperx;
...
private void InitCellContentOnThread() {
            ...
            while(true) {
                ...
                try {
                    for(var c = 0; c < wt_cellindexListCount; c++) {
...
//  The function which runs on the worker thread.
        public float InitCellContent(int cellIndex) {
            ...
            var index = 0;
            var layer = 0;
            var loopCounter = CurrentCell.CellContentCount;

            // Loop over all Cell Contents
            for (var c = 0; c < loopCounter; c ++) {
                ...
                //  Here we have to overwrite the initial value
                    ...
                    var tempMinSize = MinSize[layer];
                    var tempMaxSize = MaxSize[layer] - tempMinSize;
                ...
                //  Outer loop for softly merged layers
                    ...
                    for (var layers = 0; layers < outerLoop; layers ++) {
                    ...
                    //  Tweak layer to point at softly merged Layer
                        ...
                        for(var x = 0; x < NumberOfBucketsPerCell; x++) {
                            for(var z = 0; z < NumberOfBucketsPerCell; z++) { 
...
*/
                            ...
                            //  Now that we have the density for the Bucket we spawn x instances based on density within the given Bucket.
                                ...
                                //  Random Offsets
                                    var rand = GetATGRandomNext();

                                    var XOffset = rand * BucketSize;
                                    rand = GetATGRandomNext();

                                    var ZOffset = rand * BucketSize; 
                                ...
                                //  Get bilinear filtered height value, see: https://en.wikipedia.org/wiki/Bilinear_interpolation
                                    var normalizedHeightPos_x = normalizedPos.x * (TerrainHeightmapWidth  - 1); // "- 1" because we are working with indexes here
                                ...
                                //  To better match the geoemtry (triangles...) we only use 3 sample points and no bilinear sampling.
                                    ...
                                    var HeightLowerLeft  = TerrainHeights [ normalizedHeightPosLower_x + normalizedHeightPosLower_y ];
                                    var HeightUpperRight = TerrainHeights [ normalizedHeightPosUpper_x + normalizedHeightPosUpper_y ];
                                    if( Lower_x > Lower_y) {
                                        // we use the lower right sample
                                        //      11
                                        //      |
                                        // 00---10
                                        var HeightLowerRight  = TerrainHeights [ normalizedHeightPosUpper_x + normalizedHeightPosLower_y ];
                                        ...
                                        // |
                                        // 00
                                        var HeightUpperLeft  = TerrainHeights [ normalizedHeightPosLower_x + normalizedHeightPosUpper_y ];
                                ...
                                //  normal.Normalize(); // Takes about 0.77ms for 593 calls! – doing it manually only takes 0.14ms
                                    var lengthSqr = normal.x * normal.x + normal.y * normal.y + normal.z * normal.z;
                                ...
                                //  Set scale
                                    var scale = tempMinSize + Mathf.PerlinNoise(tempPosition.x * noise, tempPosition.z * noise) * tempMaxSize;
...
*/
                                ...
                                //  Now add random rotation around y axis
                                    var HalfAngle = GetATGRandomNext() * 270.0f; // 180.0f;
                                ...
                                //  Rotation around x axis (for e.g. small stones)
                                    if(Rotation == 1) {
                                        ...
                                        var SinX = (float)Math.Sin(HalfAngle); // --> rhs.x
                                        var CosX = (float)Math.Cos(HalfAngle); // --> rhs.w
...
// ---------------
        ...
        private void OnDrawGizmos () {
                ...
                //  cached cells
                    ...
                    for (var i = 0; i < NumberOfCells*NumberOfCells; i ++) {
                        CurrentCell = Cells[ i ];
