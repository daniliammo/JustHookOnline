...
public class EnviroSky : EnviroCore
{
    ...
    private void Start()
    {
        ...
        if (profileLoaded)
        {
            InvokeRepeating(nameof(UpdateEnviroment), 0, qualitySettings.UpdateInterval);// Vegetation Updates
...
//#endif
        ...
        {
            var systems = Components.particleClouds.GetComponentsInChildren<ParticleSystem>();
    ...
    private void SetupMainLight()
    {
        ...
        {
            var oldLight = GameObject.Find("Enviro Directional Light");
    ...
    private void SetupAdditionalLight()
    {
        ...
        {
            var oldLight = GameObject.Find("Enviro Directional Light - Moon");
    ...
    private void Update()
    {
        ...
        if (!started && !serverMode)
        {
            ...
            if (AssignInRuntime && PlayerTag != "" && CameraTag != "" && Application.isPlaying)
            {
                ...
                var plr = GameObject.FindGameObjectWithTag(PlayerTag);
                if (plr != null)
                    Player = plr;
    ...
    public void UpdateSkyShaderVariables(Material skyMat)
    {
        if (skySettings.skyboxMode == EnviroSkySettings.SkyboxModi.Simple)
        {
            ...
            {
                var angle = Vector3.SignedAngle(Components.Moon.transform.forward, Components.Sun.transform.forward, -transform.forward);
            ...
            if (skySettings.starsTwinklingRate > 0.0f)
            {
                ...
                var rot = Quaternion.Euler(starsTwinklingRot, starsTwinklingRot, starsTwinklingRot);
    ...
    {
        var fogClr = Color.Lerp(fogSettings.simpleFogColor.Evaluate(GameTime.solarTime), customFogColor, customFogIntensity);
    ...
    private void UpdateEffectSystems(EnviroWeatherPrefab id, bool withTransition)
    {
        if (id != null)
        {
            ...
            for (var i = 0; i < id.effectSystems.Count; i++)
            {
                ...
                // Set EmissionRate
                var val = Mathf.Lerp(EnviroSkyMgr.instance.GetEmissionRate(id.effectSystems[i]), id.effectEmmisionRates[i] * qualitySettings.GlobalParticleEmissionRates, speed) * interiorZoneSettings.currentInteriorWeatherEffectMod;
            ...
            for (var i = 0; i < Weather.WeatherPrefabs.Count; i++)
            {
                ...
                {
                    for (var i2 = 0; i2 < Weather.WeatherPrefabs[i].effectSystems.Count; i2++)
                    {
                        var val2 = Mathf.Lerp(EnviroSkyMgr.instance.GetEmissionRate(Weather.WeatherPrefabs[i].effectSystems[i2]), 0f, speed * 10);
    ...
    public void PopulateCloudsQualityList()
    {
        ...
        if (cloudsQualityPresetsFound.Length > 0)
        {
            ...
            for (var i = 0; i < cloudsQualityPresetsFound.Length; i++)
            {
                cloudsQualityPresetsFound[i] = UnityEditor.AssetDatabase.GUIDToAssetPath(cloudsQualityPresetsFound[i]);
                var preset = UnityEditor.AssetDatabase.LoadAssetAtPath<EnviroVolumeCloudsQuality>(cloudsQualityPresetsFound[i]);
    ...
    {
#if ENVIRO_HD
        for (var i = 0; i < cloudsQualityList.Count; i++)
        {
