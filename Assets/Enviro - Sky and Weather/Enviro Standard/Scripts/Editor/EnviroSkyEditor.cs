...
public class EnviroSkyEditor : Editor
{
    ...
    private SerializedProperty AdditionalDirectLight, Sun, Moon, DirectLight, GlobalReflectionProbe, windZone, LightningGenerator, satellites, starsRotation;
    private SerializedProperty Player, Camera, PlayerTag, CameraTag, AssignOnRuntime, tonemapping, SatTag, startMode, singlePassInstancedVR, floatingPointOriginAnchor;
    private SerializedProperty cycleLenghtInMinutes, ProgressMode, Years, Days, Hours, Minutes, Seconds, Longitude, Latitude, DayLength, NightLength, UTC, UpdateSeason, CurrentSeason, DaysInYear;
    private SerializedProperty UpdateWeather, StartWeather, EnableVolumeLighting, EnableSunShafts, EnableMoonShafts, AmbientVolume, WeatherVolume;
    private SerializedProperty lightIntensityLuxMult, exposure, angleOffset, lightColorGradient, lightIntensityCurveSun, lightIntensityCurveMoon, shadowStrength, VolumeLightingResolution, globalReflectionsUpdateOnPosition, globalReflectionsUpdateOnGameTime;
    private SerializedProperty stopRotationAtHigh, rotationStopHigh, directionalLightMode, lightIntensityTransitionSpeed, ambientMode, ambientIntensityCurve, ambientSkyGradient, ambientEquatorGradient, ambientGroundGradient;
    private SerializedProperty updateDefaultEnvironmentReflections, globalReflectionLayers, globalReflectionResolution, reflectionCloudsQuality, useFogInReflection, globalReflectionsScale, reflectionBool, reflectionIntensity, reflectionUpdate;
    private SerializedProperty sunDiskSizeSimple, simpleGroundColor, simpleSunColor, simpleSkyColor, simpleHorizonColor, simpleHorizonBackColor, renderMoon, moonGlowSize, blackGroundMode, galaxyCubeMap, galaxyIntensity, starsTwinklingRate, skyboxMode, customSkyboxMaterial, customSkyboxColor, rayleigh, g, mie, scatteringCurve, scatteringColor, sunMoonPos, sunIntensity, sunDiskScale, sunDiskIntensity, sunDiskColor, moonPhaseMode, moonTexture, moonGlowTexture, moonSize, moonGlow, currentMoonPhase, skyLuminance, skyColorPower, skyExposure, fogExposure, starsCubemap, starsIntensity, moonColor, moonGlowColor;
    private SerializedProperty cloudsSkyFogHeightBlending,dualLayerParticleClouds, setEnviroSkybox, skyboxExposure, controlSceneExposure, lightingVarianceTiling, useLessSteps, useHaltonRaymarchOffset, attenuationClamp, volumeCloudsAmbientColor, cloudsHeightMod, shadowCookieSize, shadowIntensity, customWeatherMap, silverLiningIntensity, silverLiningSpread, weatherAnimSpeedScale, globalCloudCoverage, cirrusCloudsAltitude, cloudsWorldScale, hgPhase, cloudsExposure, cirrusCloudsColor, volumeCloudsColor, volumeCloudsMoonColor, cirrusCloudsTexture, weatherMapTiling, LightIntensity, depthBlending, bilateralUpsampling;
    private SerializedProperty useTag, wetnessAccumulationSpeed, wetnessDryingSpeed, snowAccumulationSpeed, snowMeltingSpeed, cloudTransitionSpeed, fogTransitionSpeed, effectTransitionSpeed, audioTransitionSpeed, useWindZoneDirection, windTimeScale, windIntensity, windDirectionX, windDirectionY;
    private SerializedProperty useSimpleFog, simpleFogColor, fogMie, fogG, fogmode, distanceFog, startDistance, distanceFogIntensity, maximumFogIntensity, heightFog, height, heightFogIntensity, useNoise, noiseIntensity, noiseIntensityOffset, noiseScale, fogDithering, skyDithering;
    private SerializedProperty resolution, screenBlendMode, useDepthTexture, lightShaftsColorSun, lightShaftsColorMoon, treshholdColorSun, treshholdColorMoon, blurRadius, shaftsIntensity, maxRadius;
    private SerializedProperty SpringStart, SpringEnd, SummerStart, SummerEnd, AutumnStart, AutumnEnd, WinterStart, WinterEnd;
    private SerializedProperty effectQuality, updateInterval, lightningEffect, lightningRange, lightningHeight;
    private SerializedProperty setCameraClearFlags, reflectionUpdatePos;
    private SerializedProperty useFog, particleClouds, useVolumeClouds, useDistanceBlur, useFlatClouds, useParticleClouds, particleCloudsHeight1, particleCloudsColor1, particleCloudsHeight2, particleCloudsColor2, ambientLightIntensity;
    private SerializedProperty volumeLighting, SampleCount, ScatteringCoef, ExtinctionCoef, Anistropy, MaxRayLength, VolumeResolution, globalVolumeIntensity;
    private SerializedProperty showVolumeCloudsEditor, showDistanceBlurInEditor, showVolumeLightingEditor, showFogEditor, showFlatCloudsEditor, volumeNoiseIntensity, volumeNoiseScale, volumeNoiseIntensityOffset, windUpwardsIntensity, cirrusWindIntensity;
    private SerializedProperty antiFlicker, highQuality, radius, globalReflectionClouds, globalReflectionTimeSlicing;
    private SerializedProperty springBaseTemperature, summerBaseTemperature, autumnBaseTemperature, winterBaseTemperature, temperatureChangingSpeed, dayNightSwitch, snowMeltingTresholdTemperature, windIntensityTransitionSpeed;
    private SerializedProperty useAurora, auroraIntensity, auroraColor, auroraBrightness, auroraContrast, auroraHeight, auroraScale, auroraSteps, auroraSpeed;

    private SerializedProperty flatCloudsBaseTexture, flatCloudsDetailTexture, flatCloudsDirectLightColor, flatCloudsAmbientLightColor, flatCloudsAltitude, flatCloudsBaseTextureTiling, flatCloudsDetailTextureTiling, cloudsDetailWindIntensity;
    //HDRP
    private SerializedProperty ambientColorMod, indirectLightingUpdateMode,fogColorTint,exposurePhysical, skyExposurePhysical, lightingUpdateEachFrames, useEnviroFog, useHDRPFog, usePhysicalBasedLighting, sunIntensityLux, moonIntensityLux, lightColorTemperature, lightColorTint;
    private ReorderableList thunderSFX;
    ...
    private void OnEnable()
    {
        ...
        thunderSFX.drawElementCallback =
            (Rect rect, int index, bool _, bool _) =>
            {
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (myTarget.profile != null && myTarget.profile.version != latestVersion)
            ...
            {
                if (EnviroProfileCreation.UpdateProfile(myTarget.profile, myTarget.profile.version, latestVersion))
                ...
                switch (myTarget.profile.viewMode)
                {
                    ...
                    case EnviroProfile.settingsMode.Clouds:
                        ...
                        if (myTarget.cloudsQualityList.Count > 0)
                        {
                            ...
                            for (var i = 0; i < myTarget.cloudsQualityList.Count; i++)
                            {
                    ...
                    case EnviroProfile.settingsMode.Satellites:
                        ...
                        for (var i = 0; i < myTarget.satelliteSettings.additionalSatellites.Count; i++)
                        {
        ...
        if (myTarget.profile != null)
        {
            ...
            if (myTarget.profile.showWeatherUI)
            {
                ...
                if (Application.isPlaying)
                {
                    ...
                    {
                        var zonePrefabs = new GUIContent[myTarget.Weather.weatherPresets.Count];
                        for (var idx = 0; idx < zonePrefabs.Length; idx++)
                        {
                            zonePrefabs[idx] = new GUIContent(myTarget.Weather.weatherPresets[idx].Name);
                        }
                        var weatherID = EditorGUILayout.Popup(new GUIContent("Current Weather"), myTarget.GetActiveWeatherID(), zonePrefabs);
