...
public class EnviroSkyRendering : MonoBehaviour
{
    ...
    private void SetupVolumeFog()
    {
        ...
        {
            var shaderLight = Shader.Find("Enviro/Standard/VolumeLight");
            if (shaderLight == null)
    ...
    {
        var width = myCam.pixelWidth;
        var height = myCam.pixelHeight;
        ...
        {
            var halfResDescriptor = descriptor;
            ...
            var halfResDepthDescriptor = halfResDescriptor;
        ...
        if (EnviroSky.instance.volumeLightSettings.Resolution == VolumtericResolution.Quarter)
        {
            var quarterResDescriptor = descriptor;
            ...
            var quarterResDepthDescriptor = quarterResDescriptor;
    ...
    private void CreateFogMaterial()
    {
        ...
        if (!useFog)
        {
            var shader = Shader.Find("Enviro/Standard/EnviroFogRenderingDisabled");
            if (shader == null)
        ...
        {
            var shader = Shader.Find("Enviro/Standard/EnviroFogRendering");
            if (shader == null)
        ...
        {
            var shader = Shader.Find("Enviro/Standard/EnviroFogRenderingSimple");
            if (shader == null)
    ...
    private void OnPreRender()
    {
        ...
        if (useVolumeLighting)
        {
            ...
            var projLeft = Matrix4x4.Perspective(myCam.fieldOfView, myCam.aspect, 0.01f, myCam.farClipPlane);
            var projRight = Matrix4x4.Perspective(myCam.fieldOfView, myCam.aspect, 0.01f, myCam.farClipPlane);
    ...
    public void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        */
        ...
        {
            var tempTexture = RenderTexture.GetTemporary(desc);
            var tempTexture2 = RenderTexture.GetTemporary(desc);
    ...
    private void RenderVolumeClouds(RenderTexture src, RenderTexture dst)
    {
        ...
        var downsampling = EnviroSky.instance.cloudsSettings.bilateralUpsampling ? reprojectionPixelSize * usedCloudsQuality.cloudsRenderResolution : 1;
        ...
        if (downsampling > 1)
        {
            ...
            var lowDepth = DownsampleDepth(Screen.width, Screen.height, src, downsample, downsampling);

            compose.SetTexture("_CameraDepthLowRes", lowDepth);

            var upsampledTex = RenderTexture.GetTemporary(myCam.pixelWidth / downsampling * 2, myCam.pixelHeight / downsampling * 2, 0, RenderTextureFormat.DefaultHDR, RenderTextureReadWrite.Default);
            upsampledTex.filterMode = FilterMode.Bilinear;

            // composite to screen
            var pixelSize = new Vector2(1.0f / lowDepth.width, 1.0f / lowDepth.height);
    ...
    private void RenderVolumeFog(RenderTexture src, RenderTexture dst)
    {
        ...
        var dx11 = SystemInfo.graphicsShaderLevel > 40;
        ...
        {
            var _light = EnviroSky.instance.Components.DirectLight.GetComponent<Light>();
            var pass = 4;
        ...
        if (EnviroSky.instance.volumeLightSettings.Resolution == VolumtericResolution.Quarter)
        {
            var temp = RenderTexture.GetTemporary(_quarterVolumeLightTexture.descriptor);
        ...
        {
            var temp = RenderTexture.GetTemporary(_halfVolumeLightTexture.descriptor);
        ...
        {
            var temp = RenderTexture.GetTemporary(_volumeLightTexture.descriptor);
    ...
    private void RenderDistanceBlur(RenderTexture source, RenderTexture destination)
    {
        ...
        var renderDescriptor = source.descriptor;
        ...
        {
            var lastDescriptor = last.descriptor;
        ...
        for (var level = iterations - 2; level >= 0; level--)
        {
            ...
            var baseDescriptor = basetex.descriptor;
    ...
    private Mesh CreateSpotLightMesh()
    {
        ...
        var vertices = new Vector3[2 + segmentCount * 3];
        var colors = new Color32[2 + segmentCount * 3];

        vertices[0] = new Vector3(0, 0, 0);
        vertices[1] = new Vector3(0, 0, 1);

        float angle = 0;
        var step = Mathf.PI * 2.0f / segmentCount;
        var ratio = 0.9f;
        ...
        var indices = new int[segmentCount * 3 * 2 + segmentCount * 6 * 2];
        var index = 0;

        for (var i = 2; i < segmentCount + 1; ++i)
        {
            indices[index++] = 0;
            indices[index++] = i;
            indices[index++] = i + 1;
        ...
        //------------
        for (var i = 2 + segmentCount; i < segmentCount + 1 + segmentCount; ++i)
        {
            indices[index++] = i;
            indices[index++] = i + segmentCount;
            indices[index++] = i + 1;
        ...
        ////-------------------------------------
        for (var i = 2 + segmentCount * 2; i < segmentCount * 3 + 1; ++i)
        {
            indices[index++] = 1;
            indices[index++] = i + 1;
            indices[index++] = i;
    ...
    public void SetCloudProperties()
    {
        ...
        switch (myCam.stereoActiveEye)
        {
            ...
            case Camera.MonoOrStereoscopicEye.Left:
                ...
                var inverseProjectionLeft = projection.inverse;
                ...
                if (myCam.stereoEnabled)
                {
                    var inverseProjectionRightSP = myCam.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right).inverse;
            ...
            case Camera.MonoOrStereoscopicEye.Right:
                ...
                var inverseProjectionRight = projection.inverse;
        ...
        var bottomH = usedCloudsQuality.bottomCloudHeight + EnviroSky.instance.cloudsSettings.cloudsHeightMod;
        var topH = usedCloudsQuality.topCloudHeight + EnviroSky.instance.cloudsSettings.cloudsHeightMod;
    ...
    private void SetBlitmaterialProperties()
    {
        ...
        if (myCam.stereoEnabled)
        {
            var inverseProjectionSPVR = projectionSPVR.inverse;
    ...
    {
        var offset = new Vector2(1.0f / X, 1.0f / X);
        X /= downsampleFactor;
        Y /= downsampleFactor;
        var lowDepth = RenderTexture.GetTemporary(X, Y, 0);
    ...
    private void FinalizeFrame()
    {
        ...
        var reproSize = reprojectionPixelSize * reprojectionPixelSize;
        subFrameNumber = frameList[renderingCounter % reproSize];
    }
    private bool UpdateFrameDimensions()
    {
        //Add downsampling
        var newFrameWidth = myCam.scaledPixelWidth / EnviroSky.instance.cloudsSettings.cloudsQualitySettings.cloudsRenderResolution;
        var newFrameHeight = myCam.scaledPixelHeight / EnviroSky.instance.cloudsSettings.cloudsQualitySettings.cloudsRenderResolution;
        ...
        var newSubFrameWidth = newFrameWidth / reprojectionPixelSize;
        var newSubFrameHeight = newFrameHeight / reprojectionPixelSize;
    ...
    private int[] CalculateFrames(int reproSize)
    {
        ...
        var i = 0;
        var reproCount = reproSize * reproSize;
        var frameNumbers = new int[reproCount];
        ...
        while (i-- > 0)
        {
            ...
            var count = (int)(UnityEngine.Random.Range(0, 1) * 1000.0f) % reproCount;
