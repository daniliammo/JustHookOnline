...
public class EnviroZone : MonoBehaviour {
	...
	public void CreateZoneWeatherTypeList ()
	{
		...
		for ( var i = 0; i < zoneWeatherPresets.Count; i++)
		{
			...
			var addThis = true;
			for (var i2 = 0; i2 < EnviroSkyMgr.instance.GetCurrentWeatherPresetList().Count; i2++)
			{
	...
	{
		var over = new List<EnviroWeatherPrefab> ();

		for (var i = 0 ; i < curPossibleZoneWeather.Count;i++)
		{
			var würfel = UnityEngine.Random.Range (0,100);
