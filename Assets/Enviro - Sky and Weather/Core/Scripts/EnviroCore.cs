...
public class EnviroCore : MonoBehaviour
{
    ...
    public bool RenderEnviroOnThisCam(Camera cam)
    {
        ...
        for (var i = 0; i < additionalCameras.Count; i++)
        {
    ...
    public void UpdateEnviroment() // Update the all GrowthInstances
    {
        ...
        {
            for (var i = 0; i < EnviroVegetationInstances.Count; i++)
            {
    ...
    public void CreateEffects(string name)
    {
        ...
        if (Weather.SFXHolder == null)
        {
            Weather.SFXHolder = Instantiate(Audio.SFXHolderPrefab, Vector3.zero, Quaternion.identity);
            Weather.SFXHolder.transform.parent = EffectsHolder.transform;
        }

        var srcs = Weather.SFXHolder.GetComponentsInChildren<EnviroAudioSource>();
    ...
    public Transform CreateDirectionalLight(bool additional)
    {
        ...
        var newLight = newGO.AddComponent<Light>();
    ...
    public void UpdateTime(int daysInYear)
    {
        if (Application.isPlaying)
        {
            ...
            var timeProgressionSpeed = GameTime.cycleLengthInMinutes;
    ...
    public void CalculateSunPosition(float d, float ecl, bool simpleMoon)
    {
        ...
        var E = M + e * Mathf.Rad2Deg * Mathf.Sin(Mathf.Deg2Rad * M) * (1 + e * Mathf.Cos(Mathf.Deg2Rad * M));
        ...
        var sin_Lat = Mathf.Sin(Mathf.Deg2Rad * GameTime.Latitude);
        var cos_Lat = Mathf.Cos(Mathf.Deg2Rad * GameTime.Latitude);
    ...
    public void CalculateMoonPosition(float d, float ecl)
    {
        ...
        var latitude = Mathf.Deg2Rad * GameTime.Latitude;
    ...
    public void CalculateSatPosition(float d, float ecl, Transform sat, Transform satRota, EnviroSatellite satSetting)
    {
        ...
        var rotation =  new Vector3(satSetting.xRot,satSetting.yRot,satSetting.zRot);
    ...
    public void CalculateStarsPosition(float siderealTime)
    {
        ...
        var starsRotation = Quaternion.Euler(90 - GameTime.Latitude, Mathf.Deg2Rad * GameTime.Longitude, 0);
    ...
    public void UpdateAmbientLight()
    {
        ...
        var intensity = Mathf.Lerp(lightSettings.ambientIntensity.Evaluate(GameTime.solarTime), lightSettings.ambientIntensity.Evaluate(GameTime.solarTime) * 0.25f, GameTime.lunarTime * (1f - GameTime.solarTime));

        switch (lightSettings.ambientMode)
        {
            case UnityEngine.Rendering.AmbientMode.Flat:
                var lightClr = Color.Lerp(lightSettings.ambientSkyColor.Evaluate(GameTime.solarTime), currentWeatherLightMod, currentWeatherLightMod.a) * intensity;
            ...
            case UnityEngine.Rendering.AmbientMode.Trilight:
                var lClr = Color.Lerp(lightSettings.ambientSkyColor.Evaluate(GameTime.solarTime), currentWeatherLightMod, currentWeatherLightMod.a) * intensity;
                ...
                var eqClr = Color.Lerp(lightSettings.ambientEquatorColor.Evaluate(GameTime.solarTime), currentWeatherLightMod, currentWeatherLightMod.a) * intensity;
                ...
                var grClr = Color.Lerp(lightSettings.ambientGroundColor.Evaluate(GameTime.solarTime), currentWeatherLightMod, currentWeatherLightMod.a) * intensity;
...
#else
                var lightClr = Color.Lerp(lightSettings.LightColor.Evaluate(GameTime.solarTime), currentWeatherLightMod, currentWeatherLightMod.a);
                MainLight.color = Color.Lerp(lightClr, interiorZoneSettings.currentInteriorDirectLightMod, interiorZoneSettings.currentInteriorDirectLightMod.a);
#endif
    ...
    {
        for (var i = 0; i < Weather.WeatherPrefabs.Count; i++)
        {
    ...
    public void PlayLightning()
    {
        ...
        var i = UnityEngine.Random.Range(0, audioSettings.ThunderSFX.Count);
    ...
    private void RenderSatCamera(Camera targetCam, Camera.MonoOrStereoscopicEye eye)
    {
    ...
    public void UpdateParticleClouds(bool active)
    {
        ...
        if (active)
        {
            ...
            if(!cloudsSettings.dualLayerParticleClouds)
            {
                ...
                var layer1Color = cloudsSettings.ParticleCloudsLayer1.particleCloudsColor.Evaluate(GameTime.solarTime) * cloudsConfig.particleLayer1Brightness;
            ...
            else
            {
                ...
                //Layer2
                var layer2Color = cloudsSettings.ParticleCloudsLayer2.particleCloudsColor.Evaluate(GameTime.solarTime) * cloudsConfig.particleLayer2Brightness;
    ...
    public void CreateSatellite(int id)
    {
        ...
        var sat = Instantiate(satelliteSettings.additionalSatellites[id].prefab, satRot.transform);
    ...
    public void CheckSatellites()
    {
        ...
        var childs = Components.satellites.childCount;
        ...
        for (var i = 0; i < satelliteSettings.additionalSatellites.Count; i++)
        {
            CreateSatellite(i);
        }
    }
    ...
    public void CalculateSatPositions(float siderealTime)
    {
        for (var i = 0; i < satelliteSettings.additionalSatellites.Count; i++)
        {
            var satRotation = Quaternion.Euler(90 - GameTime.Latitude, GameTime.Longitude, 0);
