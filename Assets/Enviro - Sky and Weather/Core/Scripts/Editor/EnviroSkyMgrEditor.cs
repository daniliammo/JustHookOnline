...
public class EnviroSkyMgrEditor : Editor {
...
#if ENVIRO_HD
    private GUIContent[] bakingResNames = new[] { new GUIContent("64"), new GUIContent("128"), new GUIContent("256"), new GUIContent("512"), new GUIContent("1024"), new GUIContent("2048") };
    private int[] bakingResValues = new[] {64,128,256,512,1024,2048 };


    public void SelectPathForBaking(int res)
    {
        var path = EditorUtility.SaveFilePanelInProject("Save your baked sky cubemap.", "New BakedSkyCubemapHDR", "exr", "");
    ...
    private IEnumerator BakeNextFrame(int res, string path)
    {
        var volumeLight = myTarget.useVolumeLighting;
    ...
    public void BakeToCubemap(int res,string path)
    {
        ...
        {
            var current = 0;
            var tex = new Texture2D(res * 6, res, TextureFormat.RGBAFloat, false);
            ...
            {
                current++;
                var face = myTarget.Components.GlobalReflectionProbe.BakeCubemapFace(i, res);
                RenderTexture.active = face;
                tex.ReadPixels(new Rect(0, 0, res, res), res * i, 0);
                EditorUtility.DisplayProgressBar("Baking Enviro Sky", "Currently Baking. Please Wait!", current / 6);
            }
            tex.Apply();
            ...
            var assetTex = AssetDatabase.LoadAssetAtPath(path,typeof(Texture)) as Texture;

            if (assetTex == null)
                return;

            var assetpath = AssetDatabase.GetAssetPath(assetTex);
    ...
    public void ForceAddDefineSymbol(string symbol)
    {
        if(symbol == "ENVIRO_HD")
        {
            var path = AssetDatabase.FindAssets("Enviro Standard");
        ...
        {
            var path = AssetDatabase.FindAssets("Enviro Lite");
