...
namespace Edgegap.Editor
{
 ...
 #pragma warning restore CS0414 // END MIRROR CHANGE
        ...
        protected void OnEnable()
        {
            ...
            var styleSheet = AssetDatabase.LoadAssetAtPath<StyleSheet>($"{StylesheetPath}/EdgegapWindow.uss");
            // END MIRROR CHANGE
...
#pragma warning restore CS1998 // END MIRROR CHANGE
        ...
        {
            // header logo
            var logoElement = rootVisualElement.Q<VisualElement>("header-logo-img");
            logoElement.style.backgroundImage = LogoImage;

            // clipboard button
            var copyElement = rootVisualElement.Q<VisualElement>("DeploymentConnectionCopyUrlBtn");
        ...
        private void setVisualElementsToFields()
        {
            ...
            var dropdownPlaceholder = rootVisualElement.Q<VisualElement>("MIRROR_CHANGE_PORT_HARDCODED");
            var options = Enum.GetNames(typeof(ProtocolType)).Cast<string>().ToList();
        ...
        {
            var showDeploymentConnectionStopBtn = !string.IsNullOrEmpty(_deploymentsConnectionUrlReadonlyInput.text);
            ...
            // Enable stop btn?
            var isDeployed = _deploymentsConnectionStatusLabel.text.ToLowerInvariant().Contains("deployed");
        ...
        private void syncFormWithObjectStatic()
        {
            ...
            var hasAppName = checkHasAppName();
        ...
        private void onContainerPortNumInputFocusOut(FocusOutEvent evt)
        {
            ...
            if (int.TryParse(_containerPortNumInput.value, out var port))
            {
        ...
        private void onApiTokenInputChanged(ChangeEvent<string> evt)
        {
            ...
            var apiTokenTxt = evt.target as TextField;
        ...
        private Sprite getSpriteFromBase64Str(string imgBase64Str)
        {
            ...
            try
            {
                var imageBytes = Convert.FromBase64String(imgBase64Str);

                var texture = new Texture2D(2, 2);
                texture.LoadImage(imageBytes);

                var rect = new Rect( // MIRROR CHANGE: 'new()' not supported in Unity 2020
                    x: 0.0f,
                    y: 0.0f,
                    texture.width,
                    texture.height);
        ...
        private string getBase64StrFromSprite(Sprite sprite, int maxKbSize = 200)
        {
            ...
            try
            {
                var texture = makeTextureReadable(sprite.texture);

                // Crop the texture to the sprite's rectangle (instead of the entire texture)
                var croppedTexture = new Texture2D(
                ...
                var pngTextureSizeKb = textureBytes.Length / oneKb;
                var isPngLessThanMaxSize = pngTextureSizeKb < maxKbSize;

                if (!isPngLessThanMaxSize)
                {
                    textureBytes = croppedTexture.EncodeToJPG();
                    var jpgTextureSizeKb = textureBytes.Length / oneKb;
                    var isJpgLessThanMaxSize = pngTextureSizeKb < maxKbSize;
                ...
                var base64ImageString = Convert.ToBase64String(textureBytes); // eg: "Aaabbcc=="
        ...
        private async Task verifyApiTokenGetRegistryCredsAsync()
        {
            ...
            var getRegistryCredentialsResult = await wizardApi.GetRegistryCredentials();
        ...
        private void setContainerImageRepositoryVal()
        {
            ...
            var project = _credentials?.Project ?? "";
            var appName = _appNameInput?.value.ToLowerInvariant() ?? "";
        ...
        private void SyncContainerEnablesToState()
        {
            ...
            var isApiTokenVerifiedAndContainerReady = _isApiTokenVerified && _isContainerRegistryReady;
        ...
        private async Task<bool> GetAppAsync()
        {
            ...
            var getAppResult = await appApi.GetApp(_appNameInput.value);
        ...
        private async Task createAppAsync()
        {
            ...
            var appApi = getAppApi();
            var createAppRequest = new CreateAppRequest( // MIRROR CHANGE: 'new()' not supported in Unity 2020
                _appNameInput.value,
                isActive: true,
                getBase64StrFromSprite(_appIconSpriteObj) ?? "");

            var createAppResult = await appApi.CreateApp(createAppRequest);
        ...
        private void onGetCreateApplicationResult(EdgegapHttpResult<GetCreateAppResult> result)
        {
            ...
            var isSuccess = result.IsResultCode200 || result.IsResultCode409; // 409 == app already exists
            ...
            var isCreate = result.HttpMethod == HttpMethod.Post;
            var isCreateFailAppNumCapMaxed = isCreate && !_isContainerRegistryReady && result.IsResultCode400;
        ...
        {
            var shaker = new ButtonShaker(_footerNeedMoreGameServersBtn);
        ...
        private string getFriendlyCreateAppResultStr(EdgegapHttpResult<GetCreateAppResult> createAppResult)
        {
            ...
            {
                // Error
                var resultStr = $"<b>Error:</b> {createAppResult?.Error?.ErrorMessage}";
            ...
            else if (createAppResult.IsResultCode409)
            {
                ...
                var resultStr = $"<b>Warning:</b> {createAppResult.Error.ErrorMessage}";
                coloredResultStr = EdgegapWindowMetadata.WrapRichTextInColor(
        ...
        private async Task refreshDeploymentsAsync()
        {
            ...
            var isActiveStatus = getDeploymentStatusResponse?.StatusCode != null &&
        ...
        private async Task createDeploymentStartServerAsync()
        {
            ...
            try
            {
                ...
                var getDeploymentStatusResponse = await deployApi.AwaitReadyStatusAsync(
                    createDeploymentResponse.Data.RequestId,
                    TimeSpan.FromSeconds(pollIntervalSecs));

                // Process create deployment response
                var isSuccess = createDeploymentResponse.IsResultCode200;
                if (isSuccess)
                    onCreateDeploymentOrRefreshSuccess(getDeploymentStatusResponse.Data);
                else
                    onCreateDeploymentStartServerFail(createDeploymentResponse);
        ...
        private void onCreateDeploymentOrRefreshSuccess(GetDeploymentStatusResult getDeploymentStatusResult)
        {
            ...
            var portsDataKvp = getDeploymentStatusResult.PortsDict.FirstOrDefault();
            Assert.IsNotNull(portsDataKvp.Value, $"Expected ({nameof(portsDataKvp)} from `getDeploymentStatusResult.PortsDict`)");
            var deploymentPortData = portsDataKvp.Value;
            var externalPortStr = deploymentPortData.External.ToString();
            var domainWithExternalPort = $"{getDeploymentStatusResult.Fqdn}:{externalPortStr}";

            _deploymentsConnectionUrlReadonlyInput.value = domainWithExternalPort;
            var newConnectionStatus = EdgegapWindowMetadata.WrapRichTextInColor(
        ...
        private void onCreateDeploymentStartServerFail(EdgegapHttpResult<CreateDeploymentResult> result = null)
        {
            ...
            var reachedNumDeploymentsHardcap = result != null && result.IsResultCode403;
            // END MIRROR CHANGE
        ...
        private async Task onDynamicStopServerAsync()
        {
            ...
            try
            {
                ...
                var pollIntervalSecs = TimeSpan.FromSeconds(EdgegapWindowMetadata.DEPLOYMENT_STOP_STATUS_POLL_SECONDS);
            ...
            var isStopSuccess = stopResponse.IsResultCode410;
            if (!isStopSuccess)
            {
                onDynamicStopServerAsyncFail(stopResponse.Error.ErrorMessage);
                return;
            ...
            var stoppedStr = getConnectionStoppedRichStr();
        ...
        private async Task<string> getExternalIpAddress()
        {
            ...
            var ipApi = getIpApi();
            var getYourPublicIpResponseTask = await ipApi.GetYourPublicIp();
        ...
        private void ShowBuildWorkInProgress(string title, string status)
        {
        ...
        private async Task buildAndPushServerAsync()
        {
            ...
            try
            {
                ...
                var registry = _containerRegistryUrlInput.value;
                var imageName = _containerImageRepositoryInput.value;
                var tag = _containerNewTagVersionInput.value;
                ...
                var isContainerLoginSuccess = await EdgegapBuildUtils.LoginContainerRegistry(
                ...
                // push docker image
                (var isPushSuccess, var error) = await EdgegapBuildUtils.RunCommand_DockerPush(registry, imageName, tag, status => ShowBuildWorkInProgress("Uploading Docker Image (this may take a while)", status));
                if (!isPushSuccess)
                {
                ...
                var updateAppVerReq = new UpdateAppVersionRequest(_appNameInput.value) // MIRROR CHANGE: 'new()' not supported in Unity 2020
                {
                ...
                var updateAppVersionResult = await appApi.UpsertAppVersion(updateAppVerReq);
            ...
            catch (Exception ex)
            {
                ...
                var errMsg = "Edgegapbuild and push failed";
        ...
        private void loadPersistentDataFromEditorPrefs()
        {
            ...
            if (string.IsNullOrEmpty(_deploymentsConnectionUrlReadonlyInput.text))
            {
                ...
                var hasVal = !string.IsNullOrEmpty(_deploymentsConnectionUrlReadonlyInput.value);
