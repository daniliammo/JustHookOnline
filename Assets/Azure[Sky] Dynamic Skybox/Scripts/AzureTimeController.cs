...
namespace UnityEngine.AzureSky
{
    ...
    public class AzureTimeController : MonoBehaviour
    {
        ...
        [SerializeField] private Transform m_moonTransform;

        [Tooltip("The directional light used to apply the sun and moon lighting to the scene.")]
        [SerializeField] private Transform m_directionalLight;
        ...
        [SerializeField] public int m_minute;
        ...
        [SerializeField] private float m_utc;
        ...
        [SerializeField] private List<AzureCelestialBody> m_celestialBodiesList = new();

        [Tooltip("Event triggered when the minute changes.")]
        public UnityEvent m_onMinuteChange = new();

        [Tooltip("Event triggered when the hour changes.")]
        public UnityEvent m_onHourChange = new();

        [Tooltip("Event triggered when the day changes to the next day at midnight.")]
        [SerializeField] private UnityEvent m_onDayChange = new();
        ...
        [SerializeField] private List<AzureCustomEvent> m_customEventList = new();
        ...
        private readonly string[] m_weekNameList = {
            "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"
        };

        [Tooltip("String array that stores the name of each month.")]
        private readonly string[] m_monthNameList = {
        ...
        [SerializeField]
        private string[] m_dayNumberList = {
        ...
        private void ScanCustomEventList()
        {
            ...
            for (var i = 0; i < m_customEventList.Count; i++)
            {
                if (m_customEventList[i].unityEvent == null)
                    continue;
                if (m_customEventList[i].year != m_year && m_customEventList[i].year != -1)
        ...
        private void ComputeCelestialCoordinates()
        {
            switch (m_timeSystem)
            {
                ...
                case AzureTimeSystem.Realistic:
                    ...
                    // Initializations
                    var hour = m_timeOfDay - m_utc;
                    var rad = Mathf.Deg2Rad;
                    var deg = Mathf.Rad2Deg;
                    var latitude = m_latitude * rad;
                    ...
                    // Eccentric anomaly
                    M *= rad;
                    var E = M + e * Mathf.Sin(M) * (1f + e * Mathf.Cos(M));
                    ...
                    var xs = r * Mathf.Cos(lonsun);
                    var ys = r * Mathf.Sin(lonsun);
                    //    zs = 0;

                    // To convert this to equatorial, rectangular, geocentric coordinates, compute:
                    var xe = xs;
                    var ye = ys * Mathf.Cos(ecl);
                    var ze = ys * Mathf.Sin(ecl);
                    ...
                    lonsun = (v + w) * rad;
                    var sinLongitude = Mathf.Sin(lonsun);
                    var cosLongitude = Mathf.Cos(lonsun);
                    ...
                    var xh = r * (Mathf.Cos(N) * cosLongitude - Mathf.Sin(N) * sinLongitude * Mathf.Cos(i));
                    var yh = r * (Mathf.Sin(N) * cosLongitude + Mathf.Cos(N) * sinLongitude * Mathf.Cos(i));
                    ...
                    if (m_celestialBodiesList.Count > 0)
                    {
                        ...
                        for (var index = 0; index < m_celestialBodiesList.Count; index++)
                        {
                            ...
                            switch (celestialBody.type)
                            {
                                ...
                                case AzureCelestialBody.Type.Pluto:
                                    var S = 50.03f + 0.033459652f * d;
                                    var P = 238.95f + 0.003968789f * d;

                                    S *= rad;
                                    P *= rad;

                                    var pluto_lonecl = 238.9508f + 0.00400703f * d
                                    ...
                                    pluto_lonecl *= rad;
                                    pluto_latecl *= rad;
                                    var pluto_cosLatecl = Mathf.Cos(pluto_latecl);
                            ...
                            var lonecl = Mathf.Atan2(yh, xh);
                            var latecl = Mathf.Atan2(zh, Mathf.Sqrt(xh * xh + yh * yh));

                            // Geocentric (Earth-centered) coordinates
                            var cosLatecl = Mathf.Cos(latecl);
