...
namespace UnityEngine.AzureSky
{
    ...
    public class AzureWeatherController : MonoBehaviour
    {
        ...
        [SerializeField] private AzureTimeController m_azureTimeController;
        ...
        [SerializeField] private List<AzureWeatherProfile> m_defaultWeatherProfilesList = new();
        ...
        [SerializeField] private List<AzureGlobalWeather> m_globalWeatherProfilesList = new();
        ...
        [SerializeField] private Transform m_localWeatherZoneTrigger;

        [Tooltip("List of local weather zones. Place here all the local weather zones and arrange according to its priorities.")]
        [SerializeField] private List<AzureWeatherZone> m_localWeatherZonesList = new();
        ...
        [SerializeField] private List<AzureOverrideProperty> m_overridePropertyList = new();


        // Weather profiles
        private AzureWeatherProfile m_defaultWeatherProfile;
        private AzureWeatherProfile m_currentWeatherProfile;
        private AzureWeatherProfile m_targetWeatherProfile;

        // Global weather trasition
        [SerializeField]
        private float m_weatherTransitionProgress;
        private float m_weatherRamp;
        private float m_weatherTransitionLength = 10.0f;
        private float m_weatherTransitionStart;
        private int   m_weatherIndex = -1;
        private bool  m_isWeatherChanging;
        ...
        private void EvaluateCurrentWeatherProfile()
        {
            ...
            for (var index = 0; index < overrideObject.customPropertyList.Count; index++)
            {
                switch (overrideObject.customPropertyList[index].outputType)
                {
                    case AzureOutputType.Float:
                        ...
                        for (var innerIndex = 0; innerIndex < m_overridePropertyList[index].overridePropertySetupList.Count; innerIndex++)
                        {
        ...
        /// </summary>
        private static float FloatInterpolation(float from, float to, float t)
        {
        ...
        /// </summary>
        private static Color ColorInterpolation(Color from, Color to, float t)
        {
            Color ret;
        ...
        {
            var ret = false;
            var mat = m_overridePropertyList[index].overridePropertySetupList[innerIndex].targetMaterial;
        ...
        {
            var ret = false;
            var id = m_overridePropertyList[index].overridePropertySetupList[innerIndex].targetUniformID;
            Debug.Log(id);
            if (id != 0) ret = true;
            return ret;
        }
