...
namespace UnityEngine.AzureSky
{
    public class AzureEffectsController : MonoBehaviour
    {
		...
		// Thunders
		public List<AzureThunderSettings> thunderSettingsList = new();
		...
		/// </summary>
		private static void SoundEffectController(float volume, AudioSource sound)
		{
			sound.volume = volume;
			if (volume > 0)
			{
        ...
        {
            var thunder = Instantiate(thunderSettingsList[index].thunderPrefab, thunderSettingsList[index].position, thunderSettingsList[index].thunderPrefab.rotation);
            var thunderEffect = thunder.GetComponent<AzureThunderEffect>();
