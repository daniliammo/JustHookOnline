...
namespace UnityEditor.AzureSky
{
    ...
    public class AzureWeatherControllerInspector : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            {
                drawElementCallback = (Rect rect, int index, bool _, bool _) =>
                {
                    rect.y += 2;
                    var fieldRect = new Rect(rect.x + 65, rect.y, rect.width - 65, EditorGUIUtility.singleLineHeight);
                ...
                drawElementBackgroundCallback = (rect, _, active, _) =>
                {
                    if (active)
            ...
            m_globalWeatherProfilesReorderableList = new ReorderableList(serializedObject, m_globalWeatherProfilesList, true, true, true, true)
            {
                ...
                {
                    rect.y += 2;
                    var fieldRect = new Rect(rect.x + 65, rect.y, rect.width - 131, EditorGUIUtility.singleLineHeight);
            ...
            m_overridePropertyReorderableList = new ReorderableList(serializedObject, m_overridePropertyList, false, true, false, false)
            {
                drawElementCallback = (Rect rect, int index, bool _, bool _) =>
                {
                    ...
                    var fieldRect = new Rect(rect.x + 15, rect.y, rect.width - 18, height);
                    ...
                    var listKey = element.propertyPath;
                    ...
                    else
                    {
                        ...
                        {
                            drawElementCallback = (Rect innerRect, int innerIndex, bool _, bool _) =>
                            {
                                if (element.isExpanded)
                                {
                                    var half = innerRect.width / 2;
                            ...
                            {
                                var r = new Rect(innerRect.x + 8, innerRect.y - 1, innerRect.width - 6, innerRect.height + 2);
                            ...
                            elementHeightCallback = (int _) => { return EditorGUIUtility.singleLineHeight * 6; },

                            drawElementBackgroundCallback = (innerRect, _, innerIsActive, _) =>
                            {
                    ...
                    {
                        var r = new Rect(rect.x + 14, rect.y, rect.width - 18, rect.height);
                        element.isExpanded = EditorGUI.BeginFoldoutHeaderGroup(r, element.isExpanded, GUIContent.none, "RL Header");
                        EditorGUI.EndFoldoutHeaderGroup();
                        r.y -= 2;
                        EditorGUI.Foldout(r, element.isExpanded, GUIContent.none);
                ...
                drawElementBackgroundCallback = (_, _, _, _) => { }
            };
        }
